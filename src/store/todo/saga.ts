...
export function* addTodo(action: IAddAction) {
  const { userId, content, createTime, deadline } = action.payload;

  try {
    const res: IRes = yield call(TodoAPI.addTodo, userId, content,createTime,deadline);
...
export function* searchTodo(action: ISearchAction) {
  const { userId, query, createTime, deadline } = action.payload;
  try {
    const res: IRes = yield call(TodoAPI.searchTodo, userId, query, createTime, deadline);
...
export function* updateTodoContent(action: IUpdateContentAction) {
  const { todoId, content, deadline } = action.payload;

  try {
    yield call(TodoAPI.updateTodoContent, todoId, content, deadline);
    yield put({
